...
class UpdateAttendanceScreen extends StatefulWidget {
  ...
  const UpdateAttendanceScreen(
      {Key? key, required this.subject, required this.attendanceSession})
      : super(key: key);
...
class _UpdateAttendanceScreenState extends State<UpdateAttendanceScreen> {
  ...
  void initState() {
    ...
    startTimeController.text = DateFormat('HH:mm')
        .format(DateTime.parse(widget.attendanceSession.startTime!).toLocal());
    endTimeController.text = DateFormat('HH:mm')
        .format(DateTime.parse(widget.attendanceSession.endTime!).toLocal());
    startTime = TimeOfDay.fromDateTime(DateTime.parse(widget.attendanceSession.startTime!).toLocal());
    endTime = TimeOfDay.fromDateTime(DateTime.parse(widget.attendanceSession.endTime!).toLocal());
  }
        ...
        child: Column(
          ...
          children: [
            ...
            ListView.builder(
              ...
              itemCount: widget.attendanceSession.attendances?.length,
              itemBuilder: (context, index) {
                return buildStudentCard(
                  (widget.attendanceSession.attendances![index].student)!,
                  widget.attendanceSession.attendances![index],
                );
              },
            ),
          ],
              ...
              onPressed: () async {
                if (formKey.currentState!.validate()) {
                  try {
                    ...
                    widget.attendanceSession.startTime =
                        actualStartTime.toUtc().toIso8601String();
                    widget.attendanceSession.endTime =
                        actualEndTime.toUtc().toIso8601String();

                    AttendanceSession updatedAttendanceSession =
                        await AttendanceService.update_attendance_session(
                            widget.attendanceSession);
                    ...
                    Navigator.pop(context, updatedAttendanceSession);
                  } catch (e) {
                    print(e);
